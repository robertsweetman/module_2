...
async function main() {
  console.log('🚀 Starting Irish Tenders MCP Server (HTTP)...');
  ...
  app.use(cors());
  app.use(express.text({ type: 'application/json', limit: '4mb' }));
  ...
  app.get('/', (req, res) => {
    res.send(`
      <h1>Irish Tenders MCP Server</h1>
      <p>Server is running. Try <code>/health</code> or connect an MCP client to <code>/mcp</code>.</p>
    `);
  });
  ...
  app.get('/mcp', async (req, res) => {
    ...
    try {
        ...
        console.log("✅ server.connect finished – sending initialize-result");
    } catch (error) {
  ...
  app.post('/mcp', async (req, res) => {
    console.log('← POST /mcp arrived', req.query, 'bytes', req.headers['content-length']);
    ...
    try {
        ...
        console.error('Failed to handle MCP POST message:', err);
    }
  });
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`✅ Irish Tenders MCP Server is running on http://localhost:${PORT}!`);
