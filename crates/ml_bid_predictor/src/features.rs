...
impl FeatureExtractor {
    ...
    pub fn extract_features(&self, tender: &TenderRecord) -> Result<FeatureVector> {
        ...
        // 4. ca_encoded (contracting authority)
        let ca_encoded = self.encode_contracting_authority(&tender.ca);
        
        // 5-14. TF-IDF features for key terms
        let combined_text = format!(
            "{} {}",
            tender.title,
            tender.pdf_text.as_ref().unwrap_or(&String::new())
        ).to_lowercase();
...
mod tests {
    ...
    fn create_test_tender() -> TenderRecord {
        TenderRecord {
            ...
            ca: "Health Service Executive".to_string(),
            procedure: Some("Open".to_string()),
            pdf_text: Some("We require comprehensive software development services including technical support and computer systems management.".to_string()),
            codes_count: Some(3),
