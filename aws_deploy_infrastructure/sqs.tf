...
# SQS Queue for tender processing (from scraper to postgres_dataload)
resource "aws_sqs_queue" "tender_processing_queue" {
  name                       = "tender-processing-queue"
  visibility_timeout_seconds = 900     # 15 minutes (longer than postgres_dataload timeout)
  message_retention_seconds  = 1209600 # 14 days
  receive_wait_time_seconds  = 20      # Long polling

  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.tender_processing_dlq.arn
    maxReceiveCount     = 3
  })
...
# Dead Letter Queue for failed tender processing messages
resource "aws_sqs_queue" "tender_processing_dlq" {
  name                      = "tender-processing-dlq"
  message_retention_seconds = 1209600 # 14 days

  tags = {
    Name = "Tender Processing Dead Letter Queue"
  }
}

# SQS trigger for postgres_dataload Lambda
resource "aws_lambda_event_source_mapping" "tender_processing_sqs_trigger" {
  event_source_arn = aws_sqs_queue.tender_processing_queue.arn
  function_name    = aws_lambda_function.postgres_dataload.function_name

  batch_size                         = 10 # Process up to 10 tenders at a time
  maximum_batching_window_in_seconds = 5  # Wait up to 5 seconds to batch

  scaling_config {
    maximum_concurrency = 5 # Limit concurrent processing
  }
}
